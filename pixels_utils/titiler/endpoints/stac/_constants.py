...
STAC_ENDPOINT = f"{TITILER_ENDPOINT}/stac"

URL_PIXELS_CROP = "{pixels_endpoint}/{minx},{miny},{maxx},{maxy}/{width}x{height}{format}"
...
QUERY_EXPRESSION = "expression"  # rio-tiler's math expression with asset names (e.g Asset1/Asset2).
QUERY_ASSET_BIDX = "asset_bidx"  # Per asset band math expression (e.g Asset1|1;2;3).
QUERY_ASSET_EXPRESSION = "asset_expression"  # Per asset band math expression (e.g Asset1|b1\*b2).
...
QUERY_CATEGORICAL = "categorical"  # Return statistics for categorical dataset, default is false.
QUERY_C = "c"  # Pixels values for categories.
QUERY_P = "p"  # Percentile values.
QUERY_HISTOGRAM_BINS = "histogram_bins"  # Histogram bins.
QUERY_HISTOGRAM_RANGE = "histogram_range"  # Comma (',') delimited Min,Max histogram bounds

QUERY_UNSCALE = "unscale"  # Apply dataset internal Scale/Offset.
QUERY_RESCALE = (
    "rescale"  # Comma (',') delimited Min,Max range (e.g rescale=0,1000, rescale=0,1000&rescale=0,3000&rescale=0,2000).
)
QUERY_COLOR_FORMULA = "color_formula"  # rio-color formula.
QUERY_COLORMAP = "colormap"  # JSON encoded custom Colormap.
QUERY_COLORMAP_NAME = "colormap_name"  # rio-tiler color map name.
QUERY_RETURN_MASK = "return_mask"  # Add mask to the output data. Default is True.
